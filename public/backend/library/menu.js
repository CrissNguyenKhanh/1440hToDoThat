(function ($) {
  ...
  HT.menuRowHtml = (option) => {
    ...
    let $removeBtn = $("<a>")
      ...
      let $input = $("<input>") // Correct way to create an input element
      .attr("type", "hidden") // Add a type attribute to make it a hidden input
      .val(0)
      .attr('name' ,'menu[id][]'); 
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        ...
        _this
          ...
          .find(".menu-list")
          .html("<p>Error loading menu. Please try again.</p>");
  ...
  HT.renderModelMenu = (object, menuRowClass) => {
    ...
    } class="m0 choose-menu" name="" value="${object.canonical}" id="${
      object.id
    }">`;
