...
class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function forceDeleteByCondition(array $condition = [] ,$forceDelete = true){
        $query = $this->model->newQuery();
        foreach($condition as $key => $val){
            $query->where($val[0], $val[1] , $val[2]);
        }
    ...
    public function findByWhereHas(array $condition = [] , string $relation = '',
    string $alias = '' ){
    ...
    public function findWidgetItem(array $condition = [] , int $language_id = 21,string $alias = ''){
...
public function recursiveCategory(string $paramater = '',$table =''){
   ...
   $results = DB::select($query , [$paramater]);
   return $results;
}
